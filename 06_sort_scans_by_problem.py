...
INPUT: 
    klausur_map.pkl    (data base of all scanned pdf pages)
OUTPUT:
    klausur_map_aufgabe.pkl (data base for pdf files sorted by problem)
"""
...
def add_missing_qr(missing_list, inpath, outpath):
    ...
    #         f_out.getPage(r.PDFSeite - 1).mergePage(fqr.getPage(0))
        ...
        for i, r in missing_list.iterrows():
            ...
            #                    num          Aufgabe    Seite                           
            qr.add_data(','.join([str(i[0]), str(i[1]), str(i[2])]))
            qr.make(fit=True)
            qr = qr.make_image(fill_color="black", back_color="white")   
...
def add_teilnehmer_info(klausur_map, teilnehmer):
    ...
    klausur_map = klausur_map.reset_index().set_index('num')
    for ig, g in klausur_map.groupby(level = 'num'):
        for c in teilnehmer.columns:
...
# work out batch number based on the multi-index tuple (first entry)
def batch_number(index):
    return int((index[0]-1)/batch_size+1)

def sort_by_aufgabe_gruppe(klausur_map, inpath, outpath, make_batches = False):
    ...
    for a, ma in klausur_map.groupby(level = 'Aufgabe'):
        ...
        if make_batches: # für jede Gruppe einen Unterordner
            try:
                os.makedirs(os.path.join(outpath, apath))
            except OSError:
                pass

            # determines the number of batch using exam_number()
            for g, mg in ma.groupby(by=batch_number):

                pdflen = 0
                pdf_writer = PdfFileWriter()
                outname = os.path.join(apath, 'Klausur_A{:02d}_Batch{:02d}.pdf'.format(a, g))
...
def sort_by_gruppe_teilnehmer(klausur_map, inpath, outpath):
    ...
    for g, mg in klausur_map.groupby(level = 'num'):
        print('Nummer', g)
        gpath = os.path.join(outpath, 'Nummer{:03d}'.format(g))
        try:
            os.makedirs(gpath)
        except OSError:
            pass
        
        for k, mk in mg.groupby(level = 'num'):
            pdflen = 0
            pdf_writer = PdfFileWriter()
            Nachname = '-'.join(mk.iloc[0].Nachname.split(' ')) + '_' + '-'.join(mk.iloc[0].Vorname.split(' '))
            outname = os.path.join('Nummer{:03d}'.format(g), 'Klausur_{}_{:d}.pdf'.format(Nachname, k))
...
def sort_by_gruppe_teilnehmer_jpg(klausur_map, inpath, outpath):
    ...
    for g, mg in klausur_map.groupby(level = 'num'):
        ...
        for k, mk in mg.groupby(level = 'num'):
            pdflen = 0
            Nachname = '-'.join(mk.iloc[0].Nachname.split(' ')) + '_' + '-'.join(mk.iloc[0].Vorname.split(' '))
            outname = os.path.join(outpath, 'Nummer{:03d}'.format(g), 'Klausur_{}_{:d}'.format(Nachname, k))
            try:
                os.makedirs(outname)
            except OSError:
                pass
...
def check_corrected_exams(klausur_map, path):
    ...
    for a, ma in klausur_map.groupby(level = 'Aufgabe'):
        ...
        for g, mg in ma.groupby(level = 'num'):
            fname = os.path.join(apath, 'Klausur_A{:02d}_N{:03d}.pdf'.format(a, g))
...
klausur_map.to_excel('klausur_map_aufgabe.xls')
