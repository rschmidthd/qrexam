...
This script is intended to add scans that were marked in paper.
They do not need to be sorted by problem.

The file names should contain a?? for the problem number at the start.

The files are QR-scanned and then added to the already existing
data base 'klausur_map'. (name defined below)
...
def add_Klausur_map_aufgabe(klausur_map, num, Aufgabe, Seite, PDFSeite,
                    Filename,
                    Rotation = 0,
                    verbose = False,
                    verify_integrity = True, inplace = True):
    ...
    result.loc[num, Aufgabe, Seite] = {
        ...
        'Filename': os.path.basename(Filename),
        'Rotation': Rotation,
        'PDFSeite_aufgabe': PDFSeite,
        'Filename_aufgabe': os.path.basename(Filename),
        }
...
def add_scans_to_klausur_map(klausur_map, add_map):

    for index, sequence in add_map.iterrows():

        # this will cause wrong entries if index-tuple exists already!
        add_Klausur_map_aufgabe(klausur_map,
                    int(index[0]),
                    index[1],
                    index[2],
                    sequence.PDFSeite,
    ...
    # done in place, no need to return copy
    #return klausur_map
...
path_scanned = 'add_scans'
klausur_map = pd.read_pickle('map_orig.pkl')
#klausur_map_add = pd.read_pickle('add.pkl')

#%% Sammle Informationen von QR-Codes in Klausuren
#print('Scanning QR codes')
klausur_map_add, failed = scan_Klausur_QR_map(path_scanned + '/*.pdf', verbose = False)

# adding scanned pages to klausur_map data base
add_scans_to_klausur_map(klausur_map, klausur_map_add)

print('writing to data base')
# create also an xls version for easy checking with libreoffice
klausur_map.to_pickle('new.pkl')
klausur_map.to_excel('new.xls')
